...
tailored towards Skolverket's new cirriculum and learning outcomes for the subjects Computational
...
Chapter two is presenting the background for the thesis. Why computational thinking is relevant in the scools, the purpose and goals with the project and the benefits and 
sustainability with the product. Chapter three contains the method with the possibilities to implement the idea considering the pros and cons. A propose of the tools and programming
languages used and how the data are gathered to evaluate the product. The result of our product is presented in chapter four and chapter five analyze the data wich is used to 
suggest improvements in further work with the prototype with the purpose to let it be used in the scools. 
...
last key stage very well. https://www.ida.liu.se/~frehe08/FredrikHeintzLindaMannila-skolverket-20150302.pdf.
...
One challange for the society is to get qualified teachers to deliver the new curriculum because the teachers are anxious to develop new 
learning resources and have insecurity in dealing with new and unfamiliar teaching material. K�lla: https://files.eric.ed.gov/fulltext/EJ1137876.pdf 
For this reasons the project will have some focus on the teachers and how to prepare them
to use the tools the platform provide. The UI will be scaled down as much as possible to be easy to use with few steps to set-up for more focus on the programming.
This will be in form of documents of concept,
...
for minimizing the use of enviromentally hazardous substances. Cheep components is nessearly when distributing in the scool but is not imortant when developing the 
prototype. It is taken under consideration and suggestions are given in chapter x to minimize the cost for an endproduct.
