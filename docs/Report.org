...
The data will be used to evaluate the product, and for suggestions towards future work. We will not present any statistics
from the survey, since our data is too limited and biased to produce any significant results.
...
this choice because it contains both the very cheap F4P6 chip, as well as the beefier but compatible C8T6, which
...
For our downloader unit we used the ESP32\cite{esp32} micro-controller, because it has a relatively large developer community,
...
At first we wanted to make a digit display node, similar to a Lixie display\cite{Lixie}. However, due to both space and time constraints we en1ded up making a simplified
4-LED Node instead.
