...
We provided a web-based \ac{IDE} called PicoIDE, which allows you to edit programs and upload them to Compute nodes. It
...
To get schools to help us evaluate the prototype has ben challenging, even though we were well in time, half a year in advanced, with requests.
...
tools during the development needed more space. So to keep down the work, a testnode was developed in the same serie of \acp{MCU} and used for debugging.
